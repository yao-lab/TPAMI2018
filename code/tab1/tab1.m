...
p3 = 0.4; %percentage of user with nonzero gamma

s1 = 1; %maximal standard deviation of beta
s2 = 1; %maximal standard deviation of nonzero delta
s3 = 2; %standard deviation of nonzero gamma
%sigma = 0.3; %standard deviation of gaussian noise
model_gernerate = 2;

ind1 = (rand(N_F,1)<p1);
beta = zeros(1,N_F);
beta(ind1) = randn(sum(ind1),1)*s1;

gamma = zeros(n_user,1);
ind3 = (rand(n_user,1)<p3);
gamma(ind3) = randn(sum(ind3),1)*s3;


%beta is the common coefficient
Phi=randn(n_item,N_F);
 
sample_per_user = randi([50,200],n_user,1);
delta = zeros(N_F,n_user); %delta is the personalized coefficient

for i=1:n_user
ind2 = (rand(N_F,1)<p2); 
delta(ind2,i) = randn(sum(ind2),1)*s2;
end
...
ii = randi([1 n_item], m,1);
jj = randi([1 n_item-1],m,1);
jj = jj + (jj>=ii);
...
for k=1:length(u)
    res(k,1)= gamma(u(k)) + beta*Phi(ii(k),:)' - beta*Phi(jj(k),:)' + delta(:,u(k))'*Phi(ii(k),:)' - delta(:,u(k))'*Phi(jj(k),:)';% + epsilon;
end
switch model_gernerate
